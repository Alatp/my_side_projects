...
#car_acc = pd.read_csv('/home/repl/datasets/road-accidents.csv',sep="|",skiprows=9,usecols=['state','drvr_fatl_col_bmiles','perc_fatl_speed','perc_fatl_alcohol','perc_fatl_1st_time'])
car_acc = pd.read_csv('/home/repl/datasets/road-accidents.csv',sep="|",comment='#')
...
# features = car_acc[['perc_fatl_speed', 'perc_fatl_alcohol', 'perc_fatl_1st_time']].values
...
#plt.scatter(p_comps[:, 0], p_comps[:, 1],p_comps[:,3],c=km.labels_)
...
# Calculate the number of states in each cluster and their 'num_drvr_fatl_col' mean and sum.
count_mean_sum1 = car_acc_miles.groupby('cluster').agg({'state':'count', 'num_drvr_fatl_col':['mean','sum']})
count_mean_sum1
